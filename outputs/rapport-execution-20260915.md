...
**Correctif appliqué** : le serveur MCP Playwright est reconfiguré avec l'option `--isolated` (profil temporaire, jamais le navigateur/compte personnel) et le binaire "Chrome for Testing" de Playwright (build officiel pour l'automatisation, sans Gestionnaire de mots de passe ni Safe Browsing) est désormais installé et utilisé — voir `.mcp.json`.

**Vérification finale (2026-09-16)** : `tests/story-001.spec.ts` exécuté via le test-runner classique (`npx playwright test`, navigateur isolé Chrome for Testing, sans lien avec un profil/compte personnel) — **3/3 scénarios PASS** (7,1s, 0 retry). Ceci confirme que saucedemo.com ne présentait aucun défaut applicatif : les résultats NON CONCLUANT du 15/09 étaient entièrement dus à l'environnement d'exécution non isolé. Verdict final : **PASS (3/3)**, la classification NON CONCLUANT ci-dessus est celle qui était correcte *au moment de son établissement*, avec les informations alors disponibles ; elle est conservée telle quelle pour la traçabilité de l'investigation plutôt que réécrite a posteriori.
